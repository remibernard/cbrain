...
(function() {
  ...
  function load_behaviour(event) {
    ...
    //Add tooltips to dropdown menu items with descriptions (title attribute)
    loaded_element.find('select').each(function () {
      var select = $(this);
      select.find('option').each(function () {
        var option     = $(this),
            contents   = option.attr('title'),
            tooltip_id = select.attr('id') + '_' + option.val() + '_tooltip';

        if (!contents) return;

        option
          .removeAttr('title')
          .addClass('html_tool_tip_trigger')
          .data('tool-tip-id', tooltip_id)
          .data('offset-x', select.width() + 20)
          .data('offset-y', -5);
  ...
  $(function() {
    ...
    $(document).delegate(".html_tool_tip_trigger", "mouseenter", function(event) {
      ...
      $('#' + trigger.data('tool-tip-id'))
        .show()
        .offset({
          top:  offset.top  + parseInt(trigger.data('offset-y') || '30'),
          left: offset.left + parseInt(trigger.data('offset-x') || '0')
        });

    }).delegate(".html_tool_tip_trigger", "mouseleave", function(event) {
      $('#' + $(this).data('tool-tip-id')).hide();
    });
